...
def give_heads(source: InfoCommandSource, dic: dict):
    ...
    if "count" in dic and int(dic["count"]) < 1:
        source.reply("§c§l头颅数量应为大于0的整数")
        return

    owner = source.get_info().player
    player = dic["player"]
    source.get_server().execute(
        f'execute at {owner} run give {owner} minecraft:player_head{{SkullOwner:"{player}"}} {1 if "count" not in dic else dic["count"]}'
    )
